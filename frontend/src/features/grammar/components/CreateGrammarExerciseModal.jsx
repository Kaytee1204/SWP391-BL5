...
export default function CreateGrammarExerciseModal({ onClose, onCreateSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await apiRequest('/grammar-exercises', 'POST', body);
      alert('Grammar exercise created successfully!');
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card modal-card-large" onClick={e => e.stopPropagation()} style={{ maxWidth: '680px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <div>
            <h3 style={{ fontSize: '1.35rem', fontWeight: 800, color: 'var(--text-heading)' }}>
              📝 Add Grammar Multiple-Choice Exercise
            </h3>
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
              Create multiple-choice grammar quizzes for JLPT N5 - N1 learners
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.95rem' }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 700, marginBottom: '0.25rem' }}>
              Grammar Explanation & Notes (Shown to students after answering)
            </label>
            <textarea
              placeholder="Explain why the answer is correct and provide translation or usage rules..."
              value={explanation}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.6rem', marginTop: '0.75rem' }}>
            ...
            <button type="submit" disabled={loading} className="btn-dash btn-dash-primary">
              {loading ? 'Creating...' : '✨ Save Exercise'}
            </button>
          </div>
        </form>
      </div>
