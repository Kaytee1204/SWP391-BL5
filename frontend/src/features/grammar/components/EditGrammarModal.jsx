...
export default function EditGrammarModal({ pattern, onClose, onUpdateSuccess }) {
  const [jlptLevel, setJlptLevel] = useState(pattern.jlptLevel || 'N5');
  const [title, setTitle] = useState(pattern.title || '');
  const [structure, setStructure] = useState(pattern.structure || '');
  const [usageNote, setUsageNote] = useState(pattern.usageNote || '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await apiRequest(`/grammar-patterns/${pattern.patternId}`, 'PUT', body);
      alert('Grammar pattern updated successfully!');
      if (onUpdateSuccess) onUpdateSuccess(res.data);
      onClose();
    } catch (err) {
      setError(err.message || 'Failed to update grammar pattern');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="modal-card modal-card-large" onClick={e => e.stopPropagation()} style={{ maxWidth: '620px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <div>
            <h3 style={{ fontSize: '1.35rem', fontWeight: 800, color: 'var(--text-heading)' }}>
              ✏️ Edit Grammar Pattern
            </h3>
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
              Pattern #{pattern.patternId} • Last updated {new Date(pattern.updatedAt || pattern.createdAt).toLocaleDateString()}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.95rem' }}>
          ...
          <div>
            ...
            <textarea
              placeholder="Describe usage rules, nuances, and sample sentences..."
              value={usageNote}
