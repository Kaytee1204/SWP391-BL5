 ...
 * NGHIỆP VỤ: Chỉnh sửa tài khoản người dùng bởi Quản lý (Manager)
 * CHỨC NĂNG:
 *  1. Cập nhật Họ tên, Email, Cấp độ JLPT, Avatar.
 *  2. Đổi Vai trò người dùng (Thăng chức: Student ➔ Lecturer / Author).
 *  3. Đổi Trạng thái tài khoản (Active ⮂ Inactive).
 *  4. Đặt lại Mật khẩu mới cho tài khoản (bỏ trống nếu không muốn đổi).
 *  5. Gửi PUT /api/v1/accounts/{accountId} lên Backend.
 * ============================================================================
 */
export default function EditAccountModal({ account, onClose, onSaveSuccess }) {
  // --- FORM STATE KHỞI TẠO TỪ DỮ LIỆU TÀI KHOẢN HIỆN TẠI ---
  ...
  const [newPassword, setNewPassword] = useState(''); // Mật khẩu mới (tùy chọn)
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  /**
   * Xử lý lưu các thay đổi của tài khoản
   */
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Chỉ gửi trường password nếu Manager có nhập mật khẩu mới
      if (newPassword.trim()) {
      ...
      onSaveSuccess(res.data); // Báo component cha cập nhật lại bảng
      onClose();               // Đóng modal
    } catch (err) {
